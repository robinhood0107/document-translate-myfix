...
_GEMMA_CHANNEL_TOKEN_RE = re.compile(r"<\|channel\>[^\r\n<]*|<channel\|>")
...
class _TrackedJsonObject(dict[str, Any]):
    """JSON object which retains duplicate-key evidence for the hard gate."""

    def __init__(self, pairs: Sequence[tuple[str, Any]]) -> None:
        super().__init__(pairs)
        seen: set[str] = set()
        self.duplicate_keys = frozenset(
            key for key, _value in pairs if key in seen or seen.add(key)
        )
...
def _strict_index(value: Any, *, label: str) -> int:
    if isinstance(value, bool) or not isinstance(value, int) or value < 0:
        raise SemanticReviewError(f"{label} must be a non-negative integer.")
    return value


def _sha256(value: Any, *, label: str) -> str:
    digest = _nonempty_string(value, label=label)
    if not _SHA256_RE.fullmatch(digest):
        raise SemanticReviewError(f"{label} must be a SHA-256 value.")
    return digest


def _validate_translation_content(content: Any, *, index: int) -> None:
    """Mirror the product's one-object JSON response contract.

    Product parsing may remove only llama.cpp channel tokens.  It never accepts
    a reasoning prelude, trailing prose, duplicate keys, or a wider schema.
    """

    if not isinstance(content, str) or not content:
        raise SemanticReviewError(f"response row {index} content is required.")
    cleaned = _GEMMA_CHANNEL_TOKEN_RE.sub("", content).strip()
    decoder = json.JSONDecoder(object_pairs_hook=_TrackedJsonObject)
    try:
        decoded, end_index = decoder.raw_decode(cleaned)
    except (TypeError, ValueError, json.JSONDecodeError) as exc:
        raise SemanticReviewError(f"response row {index} content is not JSON.") from exc
    if cleaned[end_index:].strip():
        raise SemanticReviewError(f"response row {index} content has trailing data.")
    if not isinstance(decoded, _TrackedJsonObject):
        raise SemanticReviewError(f"response row {index} content must be a JSON object.")
    if decoded.duplicate_keys:
        raise SemanticReviewError(f"response row {index} content has duplicate keys.")
    if set(decoded) != {"translation"} or not isinstance(decoded["translation"], str):
        raise SemanticReviewError(
            f"response row {index} must contain exactly one string translation field."
        )
...
def _replay_request_ledger(result: Mapping[str, Any]) -> tuple[list[Mapping[str, Any]], str]:
    ledger = _mapping(result.get("request_ledger"), label="request_ledger")
    rows = ledger.get("rows")
    if not isinstance(rows, list) or not rows:
        raise SemanticReviewError("request_ledger.rows must be a non-empty list.")
    normalized = [_mapping(row, label="request_ledger row") for row in rows]
    observed_sha256 = _sha256(ledger.get("sha256"), label="request_ledger.sha256")
    if observed_sha256 != canonical_sha256(normalized):
        raise SemanticReviewError("request_ledger.sha256 does not match its rows.")
    for expected_index, row in enumerate(normalized):
        if _strict_index(
            row.get("logical_request_index"),
            label="request_ledger logical_request_index",
        ) != expected_index:
            raise SemanticReviewError("request ledger logical request indexes are not ordered.")
        if not isinstance(row.get("model"), str) or not row["model"]:
            raise SemanticReviewError("request ledger model identity is missing.")
        for key in ("prompt_sha256", "schema_sha256", "payload_sha256"):
            _sha256(row.get(key), label=f"request_ledger.{key}")
        if isinstance(row.get("seed"), bool) or not isinstance(row.get("seed"), int):
            raise SemanticReviewError("request ledger seed is invalid.")
...
def _response_ledger(
    ...
    *,
    ...
    observed_sha256 = _sha256(ledger.get("sha256"), label="response_ledger.sha256")
    ...
    if len(normalized) != expected_request_count:
        raise SemanticReviewError("response row count does not match the request ledger.")
    for expected_index, row in enumerate(normalized):
        if _strict_index(
            row.get("logical_request_index"),
            label="response_ledger logical_request_index",
        ) != expected_index:
            raise SemanticReviewError("response ledger logical request indexes are not ordered.")
    return normalized, observed_sha256
...
def validate_translation_response(response: Mapping[str, Any], *, index: int) -> str:
    ...
    choice_index = _strict_index(choice.get("index"), label=f"response row {index} choice index")
    if choice_index != 0:
    ...
    _validate_translation_content(content, index=index)
...
def build_replay_comparison(
    *,
    ...
    baseline_requests, baseline_request = _replay_request_ledger(baseline)
    candidate_requests, candidate_request = _replay_request_ledger(candidate)
    ...
    if len(baseline_requests) != len(candidate_requests):
        raise SemanticReviewError("request row count differs between compared runs.")
    baseline_rows, baseline_response = _response_ledger(
        baseline,
        expected_request_count=len(baseline_requests),
    )
    candidate_rows, candidate_response = _response_ledger(
        candidate,
        expected_request_count=len(candidate_requests),
    )
...
def _full_auto_ledger(
    result: Mapping[str, Any],
) -> tuple[list[Mapping[str, Any]], list[Mapping[str, Any]], str, str]:
    """Validate the full-auto HTTP record without accepting a partial ledger."""

    ledger = _mapping(result.get("request_ledger"), label="request_ledger")
    if ledger.get("record_complete") is not True:
        raise SemanticReviewError("full-auto request ledger is incomplete.")
    failures = ledger.get("record_failures")
    if not isinstance(failures, list) or failures:
        raise SemanticReviewError("full-auto request ledger has record failures.")
    attempts_value = ledger.get("actual_http_attempts")
    if not isinstance(attempts_value, list) or not attempts_value:
        raise SemanticReviewError("full-auto request ledger has no HTTP attempts.")
    attempts = [_mapping(row, label="full-auto HTTP attempt") for row in attempts_value]
    for expected_index, row in enumerate(attempts):
        if _strict_index(row.get("attempt_index"), label="full-auto attempt_index") != expected_index:
            raise SemanticReviewError("full-auto HTTP attempt indexes are not ordered.")
        if not isinstance(row.get("model"), str) or not row["model"]:
            raise SemanticReviewError("full-auto request model identity is missing.")
        for key in ("prompt_sha256", "schema_sha256", "payload_sha256"):
            _sha256(row.get(key), label=f"full-auto request {key}")
        if isinstance(row.get("seed"), bool) or not isinstance(row.get("seed"), int):
            raise SemanticReviewError("full-auto request seed is invalid.")
    fixed_contract = _mapping(ledger.get("fixed_contract"), label="full-auto fixed_contract")
    translation_start_order = ledger.get("translation_start_order")
    if not isinstance(translation_start_order, list):
        raise SemanticReviewError("full-auto translation start order is invalid.")
    request_sha256 = _sha256(ledger.get("sha256"), label="request_ledger.sha256")
    if request_sha256 != canonical_sha256(
        {
            "fixed_contract": dict(fixed_contract),
            "translation_start_order": translation_start_order,
            "actual_http_attempts": attempts,
        }
    ):
        raise SemanticReviewError("full-auto request ledger hash does not match its rows.")
    response_ledger = _mapping(ledger.get("response_ledger"), label="full-auto response_ledger")
    response_rows_value = response_ledger.get("rows")
    if not isinstance(response_rows_value, list):
        raise SemanticReviewError("full-auto response ledger rows are invalid.")
    response_rows = [_mapping(row, label="full-auto response row") for row in response_rows_value]
    response_sha256 = _sha256(response_ledger.get("sha256"), label="full-auto response_ledger.sha256")
    if response_sha256 != canonical_sha256(response_rows):
        raise SemanticReviewError("full-auto response ledger hash does not match its rows.")
    if len(response_rows) != len(attempts):
        raise SemanticReviewError("full-auto response count does not match HTTP attempts.")
    canonical_responses: list[Mapping[str, Any]] = []
    for expected_index, row in enumerate(response_rows):
        if _strict_index(row.get("attempt_index"), label="full-auto response attempt_index") != expected_index:
            raise SemanticReviewError("full-auto response indexes are not ordered.")
        response = _mapping(row.get("canonical_response"), label="full-auto canonical response")
        validate_translation_response(response, index=expected_index)
        canonical_responses.append(response)
    return attempts, canonical_responses, request_sha256, response_sha256
...
def build_full_auto_comparison(
    *,
    ...
    baseline_attempts, baseline_responses, baseline_request, baseline_response = _full_auto_ledger(
        baseline
    )
    candidate_attempts, candidate_responses, candidate_request, candidate_response = _full_auto_ledger(
        candidate
    )
    if baseline_request != candidate_request:
        raise SemanticReviewError("request ledger differs between compared runs.")
    if len(baseline_attempts) != len(candidate_attempts):
        raise SemanticReviewError("full-auto HTTP attempt count differs between compared runs.")
    ...
    mismatch_indices: list[int] = []
    for index, (baseline_response_row, candidate_response_row) in enumerate(
        zip(baseline_responses, candidate_responses)
    ):
        baseline_content = validate_translation_response(baseline_response_row, index=index)
        candidate_content = validate_translation_response(candidate_response_row, index=index)
        if baseline_content != candidate_content:
            mismatch_indices.append(index)
    return {
