...
def _final_output_key(result: Mapping[str, Any]) -> str:
    ...
    hashes = result.get("page_output_sha256")
    if not isinstance(hashes, list) or not hashes:
        return ""
    normalized = [str(value or "") for value in hashes]
    if not all(_SHA256_RE.fullmatch(value) for value in normalized):
        return ""
...
def execute_pair_matrix(
    *,
    ...
    semantic_response_mismatches: list[dict[str, Any]] = []
    semantic_bindings: list[dict[str, Any]] = []
    semantic_contract_failures: list[str] = []
    ...
    for round_index in range(1, max_rounds + 1):
        ...
        pairs.append(pair)
        if (
            baseline_result.get("status") == "passed"
            and candidate_result.get("status") == "passed"
        ):
            try:
                binding = (
                    ...
                    if mode == "replay"
                    else semantic_review.build_full_auto_comparison(
                        baseline=baseline_result,
                        candidate=candidate_result,
                    )
                )
                semantic_bindings.append(binding)
                if binding["mismatch_indices"]:
                    semantic_response_mismatches.append(
                        {
                            "round": round_index,
                            "baseline": baseline.key,
                            "candidate": candidate.key,
                            "mismatch_indices": list(binding["mismatch_indices"]),
                            "comparison_binding_sha256": semantic_review.canonical_sha256(
                                binding
                            ),
                        }
                    )
            except semantic_review.SemanticReviewError as exc:
                semantic_contract_failures.append(str(exc))
        ...
        _json_write(
            output_dir / "pair-summary.json",
            {
                ...
                "semantic_response_mismatches": semantic_response_mismatches,
        ...
        if aborted_early:
            break
        if request_ledger_mismatches or upstream_mismatches or semantic_contract_failures:
            break
        if round_index < initial_rounds:
            continue
        try:
            semantic_result = semantic_review.evaluate_semantic_review(
                protocol_version=PROTOCOL_VERSION,
                stage=semantic_stage or f"{mode}-abba",
            ...
            semantic_contract_failures.append(str(exc))
            break
        if semantic_result["status"] not in {"PASS", "NOT_REQUIRED"}:
            break
    ...
    hard_contract_pass = bool(
        ...
        and not semantic_contract_failures
        and resources_ok
        and statuses_ok
        and not aborted_early
    )
    semantic_review_status = str(semantic_result.get("status", "REJECT") or "REJECT")
    promotion = bool(
        hard_contract_pass
        and semantic_review_status in {"PASS", "NOT_REQUIRED"}
        and lower > 0.0
    )
    ...
    elif semantic_review_status == "REJECT":
        decision = "REJECT"
    else:
    ...
    result = {
        ...
        "statistics": stats,
        "quality_exact": (
            not raw_response_mismatches
            and not semantic_response_mismatches
            and not upstream_mismatches
        ),
        "raw_response_exact": not raw_response_mismatches and not semantic_response_mismatches,
    ...
    _json_write(output_dir / "pair-summary.json", result)
    return result
...
def execute_fork_replay_abba(
    *,
    ...
    result = execute_pair_matrix(
        ...
        semantic_stage="fork-f16-vs-turbo4-replay",
    )
...
def execute_shipping_replay_abba(
    *,
    candidates: Mapping[str, TurboCandidate],
    payloads: Sequence[Mapping[str, Any]],
    output_dir: Path,
    ...
    """Verify the shipping b10133 control after the fork-only winner."""

    stage_dir = output_dir / "shipping-f16-vs-turbo4-replay"
    stage_dir.mkdir(parents=True, exist_ok=False)
    ...
    result = execute_pair_matrix(
        ...
        max_rounds=max_rounds,
        semantic_approval=semantic_approval,
        semantic_stage="shipping-f16-vs-turbo4-replay",
    )
    result["stage"] = "shipping-f16-vs-turbo4-replay"
    result["image_ids"] = dict(image_ids)
    result["resource_summary"] = _aggregate_resources(
        [
            result_item
    ...
    try:
        ...
        if args.mode in {"structural", "all"}:
            ...
            fork_replay = execute_fork_replay_abba(
                ...
                semantic_approval=(
                    semantic_approval if args.structural_gate is not None else None
                ),
            )
            if fork_replay["decision"] != "PASS":
                ...
                if managed_run is not None:
                    managed_run.complete(metadata={"protocol_version": PROTOCOL_VERSION, "mode": args.mode, "decision": result["decision"]})
                return _decision_exit_code(result["decision"])
            shipping_replay = execute_shipping_replay_abba(
                candidates=candidates,
                payloads=payloads,
                output_dir=output_dir,
                port=args.port,
            ...
            if shipping_replay["decision"] != "PASS":
                result = {
                    "protocol_version": PROTOCOL_VERSION,
                    "image_ids": dict(image_ids),
                    "decision": shipping_replay["decision"],
                    "stage": "shipping-f16-vs-turbo4-replay",
                    "structural": structural,
                    "fork_replay": fork_replay,
                    "shipping_replay": shipping_replay,
                    "resource_summary": shipping_replay["resource_summary"],
                    "r3_estimate": shipping_replay["r3_estimate"],
                }
                _json_write(output_dir / "final-summary.json", result)
                _print_redacted(result)
                if managed_run is not None:
            ...
            shipping_replay = structural_gate.get("shipping_replay")
            if not isinstance(shipping_replay, Mapping) or shipping_replay.get("decision") != "PASS":
                raise Turbo4BenchmarkError(
                    "Prior structural gate lacks a passing shipping F16 versus Turbo4 ABBA result."
                )
