...
CORPORA = [
    {
        ...
        "source_lang": "Chinese",
        "target_lang": "Korean",
        "reference_preset": "ocr-combo-china-reference",
        "default_candidates": [
    ...
    },
    {
        ...
        "target_lang": "Korean",
        "reference_preset": "ocr-combo-japan-reference",
        "default_candidates": [
...
def _run_compare(reference_run_dir: Path, candidate_run_dir: Path) -> dict[str, Any]:
...
def _run_reference(
    *,
    ...
    reference_dir = suite_dir / "reference" / corpus_cfg["name"]
    run_result = _run_pipeline_once(
        preset_ref=corpus_cfg["reference_preset"],
        mode="batch",
        sample_dir=ROOT / "Sample" / corpus_cfg["sample_subdir"],
        ...
        label=f"{corpus_cfg['name']}-reference",
    )
    compare_payload = _run_compare(reference_dir, reference_dir)
...
def _run_default_compare(
    *,
    ...
    reference_run_dir = ROOT / reference_result["run_dir"]
    for candidate in corpus_cfg["default_candidates"]:
        if candidate["preset"] == corpus_cfg["reference_preset"]:
            results.append(reference_result)
            continue
        label = f"{corpus_cfg['name']}-{candidate['engine'].split()[0].lower()}-default"
        run_result = _run_pipeline_once(
            preset_ref=candidate["preset"],
            mode="batch",
            sample_dir=ROOT / "Sample" / corpus_cfg["sample_subdir"],
        ...
        compare_payload = _run_compare(reference_run_dir, run_result["run_dir"])
        results.append(
            _annotate_candidate_result(
                engine=candidate["engine"],
                preset_ref=candidate["preset"],
    ...
    *,
    ...
    for step in steps:
        ...
        for candidate_spec in step["candidates"]:
            preset_name = f"{corpus_cfg['name']}-{engine.lower().replace(' ', '-').replace('+', '')}-{candidate_spec['suffix']}"
...
def run_suite(*, sample_root: Path) -> int:
    ...
    for corpus_cfg in CORPORA:
        ...
        _log(f"reference start: corpus={corpus_cfg['name']}")
        reference_result = _run_reference(suite_dir=suite_dir, corpus_cfg=corpus_cfg)
        reference_results[corpus_cfg["name"]] = reference_result
        ...
        tuning_inputs = passing_defaults[:2]
    ...
    manifest = {
        "benchmark": {
            ...
            "official_score_scope": "full-pipeline cold median elapsed_sec with geometry/quality/translation hard gate",
