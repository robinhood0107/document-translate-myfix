    ...
    try:
        ...
        if gemma_record_file is not None:
            ...
            def recorded_post(*args, **kwargs):
                ...
                # The recorder wraps the seeded request path.  Preserve the
                # exact copied payload sent by that path without mutating the
                # product-owned request object.
                if gemma_seed is not None and isinstance(request_payload, dict):
                    request_payload = dict(request_payload)
                    request_payload["seed"] = gemma_seed
                try:
