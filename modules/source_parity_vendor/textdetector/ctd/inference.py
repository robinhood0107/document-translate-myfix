...
except Exception:  # pragma: no cover - optional path only used by model2annotations
...
except Exception:  # pragma: no cover - only used by model2annotations
    find_all_imgs = None
...
def model2annotations(model_path, img_dir_list, save_dir, save_json=False):
    if find_all_imgs is None:
        raise RuntimeError("find_all_imgs is unavailable because optional source dependencies are missing")
