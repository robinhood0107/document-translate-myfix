...
from .crop_policy import PaddleCropPolicy, resolve_paddle_crop_bbox
...
class PaddleOCRVLEngine(OCREngine):
    ...
    def _extract_text_from_response(self, data: dict) -> str:
        return extract_text_from_response(
            data,
            item_extractor=self._extract_text_from_layout_item,
        )

    def _extract_text_from_layout_item(self, item: dict) -> str:
        return extract_text_from_layout_item(
            item,
            markdown_converter=self._markdown_to_text,
            pruned_extractor=self._extract_texts_from_pruned,
            text_normalizer=self._normalize_output_text,
        )

    def _extract_texts_from_pruned(self, node) -> list[str]:
        return extract_texts_from_pruned(node)

    def _resolve_bbox(self, blk: TextBlock, image: np.ndarray) -> tuple[int, int, int, int] | None:
        return resolve_paddle_crop_bbox(
            blk,
            image.shape,
            model_identity=self.MODEL_IDENTITY,
            policy=PaddleCropPolicy(
                expansion_ratio=float(self.TEXT_EXPANSION_RATIO),
            ),
        )
