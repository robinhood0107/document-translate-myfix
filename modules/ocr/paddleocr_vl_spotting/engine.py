...
from .geometry import SpottingGeometryResult, assign_spotting_regions
...
class PaddleOCRVLSpottingEngine(OCREngine):
    ...
    def _preprocess_image(
        ...
        doubled = width < self.LOW_RES_DOUBLE_THRESHOLD and height < (
            self.LOW_RES_DOUBLE_THRESHOLD
        )
        if doubled:
            request = cv2.resize(
                image,
