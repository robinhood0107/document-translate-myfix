...
- 이 저장소는 파생 포크이며, 클린룸 재작성 프로젝트가 아닙니다.
- 리팩토링, 타 언어 포팅, 일부 기능 삭제만으로 Apache 2.0 의무가 자동으로 사라지지 않습니다.
- 모델 가중치, Docker 이미지, 폰트, 데이터셋, 체크포인트는 코드 라이선스와 별도로 자체 라이선스/이용 조건을 확인해야 합니다.
- 이 저장소는 폰트 바이너리를 배포하지 않는 방향을 기본 원칙으로 삼습니다. 필요한 폰트는 사용자가 별도로 준비해야 합니다.
