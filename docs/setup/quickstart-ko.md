...
CUDA13 경로(Python cu130 + llama.cpp `server-cuda13`; 드라이버 미지원 시
CUDA12로 자동 전환하지 않고 준비 중단):
...
CUDA13 launcher는 `server-cuda13` 이미지의 `NVIDIA_REQUIRE_CUDA`와
...
- Docker 이미지: CUDA12는 `server-cuda`, CUDA13은 `server-cuda13`
- 참고 링크:
...
선택해 원본 파일 없이 복구합니다. `Reseal`은 볼륨 내용을 그대로 두고 모델
SHA-256을 재검증한 뒤 현재 이미지로 GPU 스모크를 다시 돌리고 manifest만
다시 씁니다. 실행 중인 앱은 어긋난 봉인을 보고하고 해당 setup BAT을 요구할
뿐 volume을 직접 복구하지 않습니다.
