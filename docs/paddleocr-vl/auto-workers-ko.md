...
- 제품 기본값은 그대로 `fixed`이며, benchmark winner가 나오기 전까지 기본 동작을 바꾸지 않는다.
...
환경변수가 있으면 환경변수가 우선한다. 잘못된 값은 무시되고 `fixed`로 돌아간다.

## Runtime Contract

- `parallel_workers`는 기본적으로 기존과 동일한 고정 worker 수다.
