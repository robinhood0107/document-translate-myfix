...
완료만 요구한다. 의미가 같은 번역의 final decoded pixel SHA 차이는 진단값이며 품질 실패가
아니다. GPU/RAM/shared/swap은 모든 arm에서 기록하지만 그 관측 자체가 승패를 정하지는
않는다. OOM, runtime/container 불안정, orphan, GPU 반환 미확인은 즉시 REJECT다.
