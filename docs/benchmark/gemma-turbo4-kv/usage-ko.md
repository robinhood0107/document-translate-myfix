...
`--mode structural`은 fork F16 ↔ Turbo4 ABBA 뒤 shipping F16 ↔ Turbo4 ABBA까지 순서대로
실행한다. 각 ABBA 또는 S1/S6에서 새 raw delta가 나오면 private template을 생성하고
`REVIEW_REQUIRED`로 멈춘다. 같은 stage의 hash-bound approval만 `--semantic-approval`로
제출할 수 있다. S1/S6는 passing structural gate를 `--structural-gate`로 재사용하며 S6는
passing S1 gate도 요구한다. true series 3+3은 별도 series queue adapter와 private fixture
없이는 실행하지 않는다. `--output-dir`을 생략하면 private artifact harness가 run manifest와
증거를 관리한다.
