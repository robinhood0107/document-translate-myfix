...
4. Requirement 2의 dual-resident OCR 정책을 나중에 넣으려면 현재 단일 `_active_engine` 구조를 바로 확장하기 어렵다.
...
4. Requirement 2용 selector/dual-resident 정책은 Requirement 1 성공 후에만 활성 설계로 승격한다.
...
```yaml
tools:
  workflow_mode: legacy_page_pipeline | stage_batched_pipeline
```
...
tools:
  workflow_mode: legacy_page_pipeline | stage_batched_pipeline
  ocr_stage_policy: single_engine | dual_resident_auto
```

단, `ocr_stage_policy`는 Requirement 1 성공 전에는 제품에 노출하지 않는다.
...
3. stage-batched workflow는 같은 내부 stage 함수들을 다른 순서로 호출하는 방식으로만 조합한다.
...
2. dual-resident OCR 상주 정책
3. selector rule
4. benchmark preset / raw report / suite runner의 제품 브랜치 반영

## 다음 설계 단계 체크리스트

1. `workflow_mode` UI 위치 확정
2. settings round-trip 키 이름 확정
3. `BatchWorkflowStrategy` 배치 책임 경계 초안 작성
